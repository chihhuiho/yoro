...
from vilt.utils.compute_refcoco_refcocop_refcocog_mdetr_all import make_arrow as read_ref_all_train_val
...
def read_flickr_separateGT_train(coco_path, vg_img_path, flickr_img_path, flickr_dataset_path, mdetr_anno_path, max_len=None, tokenizer=None, max_bbox=None):
    ...
    for idx in tqdm(range(len(annotation['images']))):
        ...
        if len(category_id_lst) != 0 and len(bbox_lst) != 0 and len(token_positive_lst) != 0 and len(category_id_lst) == len(bbox_lst) == len(token_positive_lst):
            anno = image_path + annotation['images'][idx]['caption']
            db.append(anno)
        else:
...
def make_arrow(coco_path, vg_img_path, flickr_img_path,flickr_dataset_path, mdetr_anno_path, ref_path, dataset_root, max_len=None, max_bbox=None, lang="bert"):
    ...
    mix_train_db = read_mix_train(coco_path, vg_img_path, flickr_img_path,flickr_dataset_path, mdetr_anno_path, max_len=None, tokenizer=tokenizer, max_bbox=None)
    #train_db = flickr_train_db + mix_train_db + ref_all_train
    train_db =  mix_train_db 
    print(len(train_db))
    print(len(set(train_db)))
    ...
    # val
    #print("Processing flickr_val_db")
    #flickr_val_db = read_flickr_separateGT_val(coco_path, vg_img_path, flickr_img_path, flickr_dataset_path, mdetr_anno_path, max_len=max_len, tokenizer=tokenizer)
    #print("Processing ref_exp_val_db")
    #ref_exp_val_db = read_refexp_val(coco_path, vg_img_path, flickr_img_path, flickr_dataset_path, mdetr_anno_path, max_len=max_len, tokenizer=tokenizer)
    #print("Processing gqa_val_db")
    #gqa_val_db = read_gqa_val(coco_path, vg_img_path, flickr_img_path, flickr_dataset_path, mdetr_anno_path, max_len=max_len, tokenizer=tokenizer)
    #val_db = flickr_val_db + ref_exp_val_db + gqa_val_db
