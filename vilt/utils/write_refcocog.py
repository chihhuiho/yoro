...
def make_arrow(root, dataset_root):
    ...
    refcocog = REFER(root, dataset='refcocog', splitBy='umd')
    for split in ["train", "val", "test"]:
        refer_ids = []
        refer_ids.extend(refcocog.getRefIds(split=split))
        refs = refcocog.loadRefs(ref_ids=refer_ids)
        db = []
        for ref_id, ref in tqdm(zip(refer_ids, refs)):
            #if split != "test":
            gt_x, gt_y, gt_w, gt_h = refcocog.getRefBox(ref_id=ref_id)
