...
from vilt.modules.objectives import compute_irtr_recall, compute_flickr_phrase_grounding_recall
from vilt.gadgets.my_metrics import Accuracy, VQAScore, Scalar, DetAcc
        ...
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                if split == "train":
                 ...
                 setattr(pl_module, f"{split}_{k}_detacc", DetAcc())
                 setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "flickr_mdetr":
                 setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "gqa_mdetr":
                 setattr(pl_module, f"{split}_{k}_loss", Scalar())
                 setattr(pl_module, f"{split}_{k}_accuracy_answer_total", Scalar())
            elif k == "copsref":
                 if split != "train":
                     setattr(pl_module, f"{split}_{k}_detacc", DetAcc())
                 setattr(pl_module, f"{split}_{k}_loss", Scalar())
            ...
            elif k == "mdetr_pretrain":
                 if split != "train":
                     setattr(pl_module, f"{split}_{k}_detacc", DetAcc())
                 setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "mdetr_pretrain_ablade":
                 if split != "train":
                     setattr(pl_module, f"{split}_{k}_detacc", DetAcc())
                 setattr(pl_module, f"{split}_{k}_loss", Scalar())
            elif k == "irtr":
...
def epoch_wrapup(pl_module):
    ...
    elif pl_module.hparams.config["loss_names"]["flickr_mdetr"] > 0 and not pl_module.training:

        if dist.is_main_process():
            flickr_res = compute_flickr_phrase_grounding_recall(pl_module)
            pl_module.logger.experiment.add_scalar(
                "recalls/R1_all", flickr_res["Recall@1_all"], pl_module.global_step
            )
            pl_module.logger.experiment.add_scalar(
                "recalls/R5_all", flickr_res["Recall@5_all"], pl_module.global_step
            )
            pl_module.logger.experiment.add_scalar(
                 "recalls/R10_all", flickr_res["Recall@10_all"], pl_module.global_step
            )
            the_metric += flickr_res["Recall@1_all"]
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            value = getattr(pl_module, f"{phase}_{loss_name}_detacc").compute()
            pl_module.log(
                f"{loss_name}/{phase}/detacc_epoch", value
            )
            getattr(pl_module, f"{phase}_{loss_name}_detacc").reset()
        elif loss_name == "mdetr_pretrain" or loss_name == "mdetr_pretrain_ablade" :
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", getattr(pl_module, f"{phase}_{loss_name}_loss").compute())
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
        ...
        elif loss_name == "flickr_mdetr":
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", getattr(pl_module, f"{phase}_{loss_name}_loss").compute())
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()

        elif loss_name == "gqa_mdetr":
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", getattr(pl_module, f"{phase}_{loss_name}_loss").compute())
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()

            value = getattr(pl_module, f"{phase}_{loss_name}_accuracy_answer_total").compute()
            pl_module.log(
                    f"{loss_name}/{phase}/accuracy_answer_total_epoch", value
            )
            getattr(pl_module, f"{phase}_{loss_name}_accuracy_answer_total").reset()

        elif loss_name == "clever_mdetr":
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", getattr(pl_module, f"{phase}_{loss_name}_loss").compute())
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
        ...
        elif loss_name == "snlive":
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", getattr(pl_module, f"{phase}_{loss_name}_loss").compute())
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()

            value = getattr(pl_module, f"{phase}_{loss_name}_accuracy").compute()
            pl_module.log(
                    f"{loss_name}/{phase}/accuracy_epoch", value
            )
...
def set_schedule(pl_module):
    ...
    #head_names = ["vqa_classifier", "nlvr2_classifier", "refcocop_classifier", "refcocop_regressor"]
