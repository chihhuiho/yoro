...
class VisionTransformer(nn.Module):
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False, use_det_fuser=False, bbox_patch_label=None):
        #_x : [B, 3, W, H]
        _, _, ph, pw = self.patch_embed.proj.weight.shape #ph:patcht height. pw:patch width

        x = self.patch_embed(_x) # x: [B , emb_dim, W/pw, H/ph]
        x_mask = (_x.sum(dim=1) != 0).float()[:, None, :, :] #x_mask : [B, 1, W, H]
        x_mask = F.interpolate(x_mask, size=(x.shape[2], x.shape[3])).long() #x_mask : [B, 1, W/pw, H/ph]
        x_h = x_mask[:, 0].sum(dim=1)[:, 0] # x_w : [W/pw]*B 
        x_w = x_mask[:, 0].sum(dim=2)[:, 0] # x_h : [H/ph]*B
        ...
        pos_embed = pos_embed.flatten(2).transpose(1, 2) # [B ,  W/pw* H/ph, emb_dim]
        x = x.flatten(2).transpose(1, 2) # [B ,  W/pw* H/ph, emb_dim]
        patch_index = (
            torch.stack(
                torch.meshgrid(
        ...
        x_mask = x_mask.flatten(1) # [B ,  W/pw* H/ph]
        ...
        if bbox_patch_label is not None:
            for b in range(B):
                ''' For pytorch version 1.5  '''
                #idx = (select[:, 0] == b).nonzero()
                idx = torch.nonzero((select[:, 0] == b), as_tuple=False)
                bbox_num = bbox_patch_label[b].shape[1]
                bbox_patch_label[b] = bbox_patch_label[b][select[idx,1], :].view(-1,bbox_num)
        ...
        if self.add_det_token == True:
            det_token = self.det_token.expand(B, -1, -1)
            if use_det_fuser:
                det_token = self.det_token_fuser(det_token.view(B, -1, 1, self.embed_dim)).view(B,-1,self.embed_dim)
            x = torch.cat((x, det_token), dim=1)
            det_pos_embed = self.det_pos_embed.expand(B, -1, -1)
            if use_det_fuser:
                det_pos_embed = self.det_pose_embed_fuser(det_pos_embed.view(B, -1, 1, self.embed_dim)).view(B,-1,self.embed_dim)
            pos_embed = torch.cat((pos_embed, det_pos_embed), dim=1)
            x_mask = torch.cat([x_mask, torch.ones(x_mask.shape[0], self.det_token_num).to(x_mask)], dim=1)


        x = torch.cat((cls_tokens, x), dim=1) # [B, W/pw* H/ph+1, emb_dim]

        # concatenate the pos_emb for cls token to the original pos_emb
        pos_embed = torch.cat( 
            (self.pos_embed[:, 0, :][:, None, :].expand(B, -1, -1), pos_embed), dim=1
        ) # [B , W/pw* H/ph+1, emb_dim]
        x = x + pos_embed
        x = self.pos_drop(x)
        ...
        x_mask = torch.cat([torch.ones(x_mask.shape[0], 1).to(x_mask), x_mask], dim=1) # [B , W/pw* H/ph+1]

        if not mask_it:
            label = None

        return x, x_mask, (patch_index, (H, W)), label, bbox_patch_label


    def truncate_det_token(self, desired_token):
        self.det_token_num = desired_token
        truncated_det_token = nn.Parameter(self.det_token[:,:desired_token, :].clone())
        delattr(self, 'det_token')
        self.det_token = truncated_det_token
        truncated_det_pos_embed = nn.Parameter(self.det_pos_embed[:,:desired_token, :].clone())
        delattr(self, 'det_pos_embed')
        self.det_pos_embed = truncated_det_pos_embed

    def fuse_tokens(self, current_token, desired_token):
        self.det_token_fuser = nn.Conv2d(current_token , desired_token, kernel_size=1, stride=1)
        self.det_pose_embed_fuser = nn.Conv2d(current_token , desired_token, kernel_size=1, stride=1)
        self.det_token_num = desired_token

    def det_init(self, det_token_num=100):
        import math
        self.add_det_token = True
        self.det_token_num = det_token_num
        self.det_token = nn.Parameter(torch.zeros(1, det_token_num, self.embed_dim))
        trunc_normal_(self.det_token, std=0.02)
        self.det_pos_embed = nn.Parameter(torch.zeros(1, det_token_num, self.embed_dim))
        trunc_normal_(self.det_pos_embed, std=0.02)

    def gqa_ans_token_init(self,):
        import math
        '''
        self.det_token_num += 6 # for gqa answer query
        reinit_det_token = torch.zeros(1, self.det_token_num, self.embed_dim)
        trunc_normal_(reinit_det_token, std=0.02)
        reinit_det_token[:,:self.det_token_num-6,:] = self.det_token.detach().clone()
        reinit_det_token = nn.Parameter(reinit_det_token)

        reinit_det_pos_embed = torch.zeros(1, self.det_token_num, self.embed_dim)
        trunc_normal_(reinit_det_pos_embed, std=0.02)
        reinit_det_pos_embed[:,:self.det_token_num-6,:] = self.det_pos_embed.detach().clone()
        reinit_det_pos_embed = nn.Parameter(reinit_det_pos_embed)
        delattr(self, 'det_token')
        delattr(self, 'det_pos_embed')
        self.det_token = reinit_det_token
        self.det_pos_embed = reinit_det_pos_embed
        '''
        delattr(self, 'det_token')
        delattr(self, 'det_pos_embed')
        self.det_init(det_token_num=self.det_token_num+6) 

    def clever_ans_token_init(self,):
        import math
        delattr(self, 'det_token')
        delattr(self, 'det_pos_embed')
        self.det_init(det_token_num=self.det_token_num+4) 

    def snlive_ans_token_init(self,):
        import math
        delattr(self, 'det_token')
        delattr(self, 'det_pos_embed')
        self.det_init(det_token_num=self.det_token_num+1) 
