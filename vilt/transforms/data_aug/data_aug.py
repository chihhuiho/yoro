...
import sys
import os
from vilt.transforms.data_aug.bbox_util import *
import torch
...
class RandomScale(object):
    ...
    def __call__(self, img_input, bboxes_dict):
    
        img = np.array(img_input.copy())
        bboxes = bboxes_dict["boxes"].numpy()
        ...
        if bboxes.shape[0] == 0: # the box is dropped after the augmentation
            return img_input, bboxes_dict
        bboxes_dict["boxes"] = torch.FloatTensor(bboxes)
        return Image.fromarray(img), bboxes_dict
