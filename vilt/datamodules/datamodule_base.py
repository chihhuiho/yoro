...
#def get_pretrained_fast_tokenizer(from_pretrained):
...
def get_pretrained_roberta_tokenizer(from_pretrained):
    if torch.distributed.is_initialized():
        if torch.distributed.get_rank() == 0:
...
class BaseDataModule(LightningDataModule):
    def __init__(self, _config):
        ...
        #self.fast_tokenizer = get_pretrained_fast_tokenizer(tokenizer)

        #for caption in [["I am so tired"], ["yeah it is the same dictionary pretrain weight"]]:
        #    tokenized = self.tokenizer(caption, return_tensors="pt")
        #    print(tokenized)
        #    fast_tokenized = self.fast_tokenizer(caption, return_tensors="pt")
    ...
    def set_train_dataset(self):
        self.train_dataset = self.dataset_cls(
            ...
            specified_train_data=self.config["specified_train_data"]
        )
    ...
    def set_val_dataset(self):
        self.val_dataset = self.dataset_cls(
            ...
            draw_false_text=self.draw_false_text,
            image_only=self.image_only,
            specified_val_data=self.config["specified_val_data"]
        )
    ...
    def set_test_dataset(self):
        self.test_dataset = self.dataset_cls(
            ...
            draw_false_text=self.draw_false_text,
            image_only=self.image_only,
            specified_test_data=self.config["specified_test_data"]
        )
